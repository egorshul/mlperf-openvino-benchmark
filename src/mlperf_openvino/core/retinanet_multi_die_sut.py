...
Multi-die accelerator support for RetinaNet object detection.
Uses NHWC input layout for NPU optimization.
"""
...
class RetinaNetMultiDieCppSUTWrapper:
    ...
    def __init__(
        ...
        if not CPP_SUT_AVAILABLE:
            raise ImportError(
                "C++ RetinaNet SUT not available. Build with: "
        ...
        # Check if using NHWC input layout (important for NPU)
        use_nhwc = False
    ...
    def run_native_benchmark(
        ...
        # For accuracy mode, load ALL samples
    ...
    def get_predictions(self) -> Dict[int, Dict[str, Any]]:
        ...
        for idx, pred in cpp_preds.items():
            # Reshape boxes from flat array to [N, 4]
            boxes = np.array(pred.get('boxes', []))
            scores = np.array(pred.get('scores', []))
            labels = np.array(pred.get('labels', []))
            num_det = pred.get('num_detections', 0)
    ...
    def compute_accuracy(self) -> Dict[str, float]:
        ...
        if not predictions:
            logger.warning(f"No predictions available")
        ...
        try:
            ...
            logger.warning(f"pycocotools evaluation failed: {e}")
...
def is_retinanet_multi_die_cpp_available() -> bool:
    """Check if C++ multi-die RetinaNet SUT is available."""
